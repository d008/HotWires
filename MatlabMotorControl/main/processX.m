...
% hold on
% semilogx(ref.y_plus,ref.U_plus,'-')
% hold off
xlabel('y^+')
ylabel('U^+')
print('mean','-dpng')
...
% hold on
% semilogx(ref.y_plus,ref.u2_plus,'-')
% hold off
xlabel('y^+')
ylabel('u^{2+}')
print('varU','-dpng')
...
print('uv','-dpng')
%%
save('acquisition.mat','utau','y_plus','meanU','u2_plus','varU','U_plus',...
    'v2_plus','varV','covUV','uv_plus','Puu','Puv','Pvv','F','-append')
%%
figure(6)
clf
[why,eff] = meshgrid(y_plus,F);
contourf(why,eff,eff.*Puu)
ax =gca;
ax.XScale = 'log';
ax.YScale = 'log';
...
cd ..
%%
umean = fit([data.D/2*1000-data.yActual';data.D/2*1000],[mean(meanU,2);0],'smoothingspline');
rad = linspace(0,data.D/2*1000,1000);

U_b = sum(2*pi.*diff(rad.^2)'.*(umean(rad(1:end-1))+diff(umean(rad))/2))./((data.D/2*1000).^2.*2*pi);
Re_D = U_b.*data.rho.*data.D./(data.Mu);


summary.Re_D = data.rho.*U_b.*data.D./data.Mu;
summary.Re_tau = data.D./eta*1000/2;
summary.U_b = U_b;
summary.utau = utau;
summary.eta = eta/1000;
summary.nu = data.Mu./data.rho;
summary.mu = data.Mu;
summary.rho = data.rho;
summary.TempK = mean(data.TempK);
summary.P_gauge_atm =  mean(data.Static_Pa)./101325;
summary.R = data.D/2;
summary.U_max = max(mean(meanU,2));
summary.k = 0.15e-6;
summary.Cf = 2*(utau/U_b).^2;
summary.y = data.yActual/1000';
summary.y_plus = y_plus';
summary.U_plus = mean(U_plus,2);
summary.u2_plus = u2_plus;
summary.v2_plus = v2_plus;
summary.uv_plus = uv_plus';


save('concise.mat','summary')